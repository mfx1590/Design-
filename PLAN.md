...
**2026-09-08 (late night) — Phase 5 built; code on GitHub; launch QA run.**

- Owner said "go ahead with phase 5" and gave the repository `https://github.com/mfx1590/Design-.git`: `main` is pushed and every commit since goes there. Vercel import + env vars + Hostinger DNS steps are in `README.md` for the owner.
- Phase 5 as adapted to the WhatsApp decision: no contact form and no Resend (forms replaced by prefilled WhatsApp links). Added: cookieless analytics switch (`NEXT_PUBLIC_ANALYTICS=vercel|plausible`, off by default), a one-line cookie note (only the language cookie exists; shown after scrolling, dismiss stored in the browser), security headers (nosniff, frame options, referrer policy, permissions policy, HSTS), `scripts/check-seo.mjs` (crawls all 198 sitemap URLs: title, description, canonical, hreflang ×7, og:image, one h1, JSON-LD parses; 0 problems on the production build), `VIEWPORTS=` option for the screenshot script.
- Lighthouse (mobile, local production server, simulated slow 4G + 4× CPU), before → after the QA fixes: Home 68 → 78 performance (LCP 11.7 s → 4.8 s), Project 65 → 85, Product 73 → 85, Guide 72 → 87; accessibility 100 on all four (was 96–100), best practices 100, SEO 92 (the missing 8 is the canonical pointing to the local site URL; resolves with `NEXT_PUBLIC_SITE_URL` on the live domain). Fixes: hero frames preload after load and idle with 3–4 workers and every other frame on phones (10 MB → 5.4 MB), poster gets `fetchpriority=high`, font preloads 11 → 7, brass blends to bronze in the light "day" state (5:1), small notes use a solid `ink-muted` token, brass buttons use surface-coloured text, one h1 per project page.
- Performance target of ≥ 90 is not met locally for the home page (78): the remaining cost is JavaScript hydration (floor-plan walk, GSAP, Lenis) under 4× CPU throttling. Re-measure on Vercel; next levers would be lazy-loading GSAP/Lenis below the fold and trimming the floor-plan walk bundle.
- Still for the owner: create the Vercel project and set `NEXT_PUBLIC_SITE_URL`; point the Hostinger DNS; create the Sanity project; tap a WhatsApp link on an iPhone and an Android phone; native-speaker read of the translations; the facts marked "to be confirmed".

**2026-09-09 — Live on Hostinger; first phone feedback.**

- Hosting is Hostinger (Node.js web app importing GitHub `main`), live at https://design.tiktecho.com. Their build could not download pnpm through corepack, so the repository now installs with npm (`package-lock.json`, no pnpm files) and the Hostinger build settings are: package manager npm, build `npm run build`, output `.next`, Node 22, env `NEXT_PUBLIC_SITE_URL=https://design.tiktecho.com`.
- Owner reported that scrolling on a phone did not feel right. Changes: native scrolling on touch devices (Lenis only for mouse wheels), `ScrollTrigger.config({ ignoreMobileResize: true })`, pinned scenes sized with `svh` instead of `dvh` so the collapsing address bar no longer resizes them, the day→dusk blend quantised to 4% steps (each step recolours the whole page), no backdrop blur on phones (header, apartment bar, scene note, card chips). Home Lighthouse mobile 78 → 85 locally (TBT 80 ms). Awaiting the owner's re-test with a precise description if anything still feels off (jumpy, laggy, or stuck on the pinned hero).
- Owner clarified the phone complaint: the hero crop showed the window and hid the furniture. The phone frame set is now a 4:3 crop of the desktop frames (`ffmpeg crop=1200:900:200:0` → 900×676, 158 frames, 6.1 MB), and on phones the scene is a 4:3 photo panel with the text below it instead of an overlay (desktop unchanged). The original Higgsfield source video is no longer on disk; any future re-cut must start from `public/sequences/project-01-studio/1600/`.
